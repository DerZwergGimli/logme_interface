...
#include "../libs/Esp32-SoftwareSerial/SoftwareSerial.h"
#include <jled.h>
#include "debug.h"
...
private:
    ...
    void run_current_state()
    {
        if (this->state != INIT)
        {
            ...
            {
                DEBUGU8("Did not receive an SML message within %d seconds, starting over.", READ_TIMEOUT);
                this->reset_state();
            }
            switch (this->state)
            {
    ...
    void set_state(State new_state)
    {
        ...
        {
            DEBUGE("State of sensor %s is 'STANDBY'.\n", this->config->name);
        }
        else if (new_state == WAIT_FOR_START_SEQUENCE)
        {
            DEBUGE("State of sensor %s is 'WAIT_FOR_START_SEQUENCE'.\n", this->config->name);
            this->last_state_reset = millis();
            this->position = 0;
        }
        else if (new_state == READ_MESSAGE)
        {
            DEBUGE("State of sensor %s is 'READ_MESSAGE'.\n", this->config->name);
        }
        else if (new_state == READ_CHECKSUM)
        {
            DEBUGE("State of sensor %s is 'READ_CHECKSUM'.\n", this->config->name);
            this->bytes_until_checksum = 3;
        }
        else if (new_state == PROCESS_MESSAGE)
        {
            DEBUGE("State of sensor %s is 'PROCESS_MESSAGE\n'.", this->config->name);
        };
